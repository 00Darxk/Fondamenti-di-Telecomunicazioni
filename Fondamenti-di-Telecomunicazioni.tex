...
\begin{gather*}
    ...
    x[n]=\cos(2\pi\phi n)\\
    \phi_1=0\to x_1[n]=1\\
    y[n]=h[n]*x[n]=\displaystyle\left[\frac{1}{4}\delta[n+1]+\frac{1}{2}\delta[n]+\frac{1}{4}\delta[n-1]\right]*x[n]\\
...
Per campionare un segnale si considere un DAC, "Digital to Analog Converter", un circuito composto da un interrutore che si chiude ad intervalli regolari per misurare il 
valore in entrata: 
\begin{center}
    \begin{tikzpicture}[scale=2]
        \node[block](c)at(0,0){DAC};
...
\subsection{Esercizi sulle Trasformate Discrete}

Data una sequenza di valore costante:
\begin{gather*}
    x[n]=1
\end{gather*}
Si determina la trasformata della sequenza:
\begin{gather*}
...
\end{gather*}
Si può riscrivere mediante i fratti semplici in una somma di due frazione:
\begin{gather*}
...
\begin{gather*}
    ...
    X_C(f)=\displaystyle\sum_{n=-\infty}^{+\infty}X\left(f-\frac{n}{T}\right)=\sum_{n=-\infty}^{+\infty}\frac{1}{2}\mbox{rect}\left(3f-\frac{1}{2}-3n\right)+\frac{1}{2}\mbox{rect}\left(3f+\frac{1}{2}-3n\right)
\end{gather*}
