...
\end{equation*}
Come la serie di Fourier la trasformata di Fourier è uno strumento per analizzare i segnanli. Questi segnali possono essere nel dominio del tempo $x(t)$ o dominio 
...
La trasformata $X(f)$ è univoca per ogni segnale $x(t)$ nel tempo. Questa trasformata trasferisce lo stesso segnale dal dominio del tempo al dominio della frequenza:  
\begin{equation*}
    \mathscr{F}\{x(t)\}=X(f)
\end{equation*}
...
Allora:
\begin{gather*}
    x(t)\to^{\mathscr{F}} X(f)\to^{\mathscr{F}}x(-t)
\end{gather*}
...
\begin{gather*}
    x(at)\to\displaystyle\frac{1}{|a|}X\left(\frac{f}{a}\right)
\end{gather*}
