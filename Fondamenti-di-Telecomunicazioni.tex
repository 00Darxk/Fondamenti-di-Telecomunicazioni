...
\begin{center}
    \begin{tikzpicture}[scale=2]
        ...
        \filldraw[black](0.25,1)circle(1pt);
...
\begin{center}
    \begin{tikzpicture}[scale=2]
        ...
        \filldraw[black](0.5,1.133)circle(1pt);
        \filldraw[black](0.5,0.882)circle(1pt);
        \filldraw[black](1,1.284)circle(1pt);
        \filldraw[black](1,0.779)circle(1pt);
        \filldraw[black](1.5,1.455)circle(1pt);
        \filldraw[black](1.5,0.687)circle(1pt);
...
Il prodotto di un qualsiasi segnale $x[n]$ per la delta equivale al valore del segnale in $0$ per la delta. Questa caratteristica viene chiamata propietà di campionamento 
...
L'area del prodotto di un qualsiasi segnale $x[n]$ per la delta risulta nel valore del segnale in $0$, ciò segue banalmente da le precedenti proprietà:
\begin{equation*}
    \displaystyle\sum_{n=-\infty}^\infty x[n]\cdot\delta[n]=x[0]
\end{equation*}



La proprietà di campionamento può comprende una traslazione nella delta, cambiano il valore del segnale $x[n]$ campionato:
\begin{equation*}
    x[n]\cdot\delta[n-m]=x[m]\delta[n-m]
\end{equation*}



La convoluzione nel discreto per una delta di una sequenza di campioni $x_i$ corrisponde al segnale discreto che li ha generati. Ovvero un qualsiasi segnale discreto $x[n]$ può 
essere espresso come una sommatoria dei suoi campioni moltiplicati ognuno per una delta traslata di un diverso fattore:
\begin{equation*}
    x[n]=\displaystyle\sum_{n=-\infty}^\infty x_i\delta[n-i]
\end{equation*}



La delta può essere rappresentta come la differenza tra due gradini:
\begin{equation*}
...
L'area descritta dalla delta è unitaria:
\begin{equation*}
...
Il prodotto di un qualsiasi segnale per la delta equivale al valore del segnale in $0$ moltiplicato per l'impulso, poiché per tutti gli altri valori del tempo questo prodotto 
è nullo:
\begin{equation*}
...
\begin{equation*}
    \displaystyle\int_{-\infty}^{+\infty}x(t)\cdot\delta(t)dt=x(0)
\end{equation*}



Il prodotto dell'impulso traslato di un fattore $\tau$ per un qualunque segnale $x(t)$ corrisponde al valore del segnale in $\tau$ moltiplicato per l'impulso traslato:
\begin{equation*}
...
\begin{equation*}
    x(t)*\delta(t)=\displaystyle\int_{-\infty}^{+\infty}x(\tau)\cdot\delta(t-\tau)d\tau=x(t)\int_{-\infty}^{+\infty}\delta(t-\tau)d\tau=x(t)
\end{equation*}
...
L'impulso è un segnale né di energia né di potenza. I funzionali, come l'impulso, vengono descritti in base agli effetti provocano sulle funzioni
